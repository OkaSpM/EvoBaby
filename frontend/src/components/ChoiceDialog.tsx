...
export function ChoiceDialog({ prompt, busy, onChoose }: { prompt: ChoicePrompt; busy: boolean; onChoose: (option: ChoiceOption) => void }) {
  ...
  return <div className="game-modal-backdrop"><section ref={dialog} className="choice-dialog" role="dialog" aria-modal="true" aria-labelledby="choice-title" onKeyDown={event => {
  ...
  }}><header><span className="micro-label">TRIBE DECISION / {prompt.point}</span><span className="choice-required">部落在等你的决定</span></header><div className="choice-intro"><PointIcon size={28} /><h2 id="choice-title">{prompt.title || fallback.title}</h2><p>{prompt.story || prompt.description || fallback.story}</p></div><div className="choice-options">{fallback.options.map((option, index) => {
